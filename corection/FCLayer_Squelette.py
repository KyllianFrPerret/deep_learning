...
class FCLayer:
    ...
    def __init__(self,nbInput,nbOutput):
        ...
        for _ in range(nbOutput):
            self.bias.append(random()-0.5)
        # self.bias = [random()-0.5 for _ in range(nbOutput)]
        ...
        for i in range(nbOutput):
            self.weight.append([])
            for _ in range(nbInput):
                self.weight[i].append(random()-0.5)

        # on crée un tableau de 0 pour les entrées
        self.input = [0]*nbInput

        # on crée un tableau de 0 pour les sorties
        self.output = [0]*nbOutput
    ...
    def forward_propagation(self, input):
        ...
        for neurone in range(len(self.weight)):
            for entreeN in range(len(self.weight[0])):
    ...
    def backward_propagation(self, output_error, learning_rate):
        ...
        # Produit matriciel : dE/dX = E fois poids (déjà stockés sous forme transposée)
        input_error = []
